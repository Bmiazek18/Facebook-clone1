...
resource "null_resource" "proxmox_host_gpu_passthrough" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "pct exec ${var.container_vm_id} -- bash -c \"rm -f /etc/apt/sources.list.d/nvidia* /usr/share/keyrings/nvidia* /etc/apt/trusted.gpg.d/nvidia*; apt-get update -y; DEBIAN_FRONTEND=noninteractive apt-get install -y gpg gnupg curl; mkdir -p /usr/share/keyrings /etc/cdi /var/run/cdi /var/lib/rancher/k3s/agent/etc/containerd; curl -fsSL https://nvidia.github.io/libnvidia-container/gpgkey | gpg --dearmor -o /usr/share/keyrings/nvidia-container-toolkit-keyring.gpg; echo 'deb [signed-by=/usr/share/keyrings/nvidia-container-toolkit-keyring.gpg] https://nvidia.github.io/libnvidia-container/stable/deb/amd64 /' > /etc/apt/sources.list.d/nvidia-container-toolkit.list; apt-get update -y; DEBIAN_FRONTEND=noninteractive apt-get install -y nvidia-container-toolkit; nvidia-ctk cdi generate --output=/etc/cdi/nvidia.yaml; nvidia-ctk cdi generate --output=/var/run/cdi/nvidia.yaml; nvidia-ctk runtime configure --runtime=containerd; cp -f /var/lib/rancher/k3s/agent/etc/containerd/config.toml /var/lib/rancher/k3s/agent/etc/containerd/config.toml.tmpl 2>/dev/null || true; nvidia-ctk runtime configure --runtime=containerd --config=/var/lib/rancher/k3s/agent/etc/containerd/config.toml.tmpl 2>/dev/null || true; systemctl restart k3s\"",
      "pct exec ${var.container_vm_id} -- /usr/local/bin/k3s kubectl label nodes --all nvidia.com/gpu.present=true --overwrite || true",
