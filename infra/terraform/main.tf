...
resource "null_resource" "proxmox_host_gpu_passthrough" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "  echo 'Instaluje oficjalny sterownik nvidia-driver...'",
      "  DEBIAN_FRONTEND=noninteractive apt-get install -y nvidia-driver nvidia-smi firmware-misc-nonfree || DEBIAN_FRONTEND=noninteractive apt-get install -y nvidia-open-kernel-dkms nvidia-smi || true",
      "fi",
