...
resource "null_resource" "proxmox_host_gpu_passthrough" {
  ...
  provisioner "remote-exec" {
    inline = [
      "set -e",
      "echo '=== [1/5] Czyszczenie i bezpieczna konfiguracja repozytoriow na Proxmox VE ==='",
      "rm -f /etc/apt/sources.list.d/pve-gpu.list /etc/apt/sources.list.d/pve-enterprise.list /etc/apt/sources.list.d/pve-enterprise.sources /etc/apt/sources.list.d/ceph.list /etc/apt/sources.list.d/ceph.sources /etc/apt/sources.list.d/ceph-squid.list /etc/apt/sources.list.d/ceph-squid.sources 2>/dev/null || true",
      "grep -rl 'enterprise.proxmox.com' /etc/apt/ 2>/dev/null | xargs -r rm -f || true",
      "CODENAME=$(grep -oP '(?<=VERSION_CODENAME=)[a-z]+' /etc/os-release 2>/dev/null || echo 'bookworm')",
      "if [ -f /etc/apt/sources.list.d/debian.sources ]; then",
      "  sed -i -E 's/Components: main( contrib)?( non-free)?( non-free-firmware)?$/Components: main contrib non-free non-free-firmware/g' /etc/apt/sources.list.d/debian.sources",
      "fi",
      ...
      "echo '=== [2/5] Blokowanie nouveau oraz instalacja naglowkow PVE i sterownikow NVIDIA ==='",
      "cat <<'EOF_NOUVEAU' > /etc/modprobe.d/blacklist-nouveau.conf",
      "blacklist nouveau",
      "options nouveau modeset=0",
      "EOF_NOUVEAU",
      "update-initramfs -u || true",
      "if ! which nvidia-smi >/dev/null 2>&1 || ! nvidia-smi >/dev/null 2>&1; then",
      "  echo 'Instaluje pve-headers i narzedzia budowy...'",
      "  DEBIAN_FRONTEND=noninteractive apt-get install -y pve-headers-$(uname -r) proxmox-headers-$(uname -r) proxmox-default-headers pve-headers dkms build-essential || true",
      "  echo 'Usuwanie konfliktujacych pakietow dystrybucyjnych...'",
      "  apt-get purge -y 'nvidia*' 'libnvidia*' 'glx-alternative-*' 2>/dev/null || true",
      "  rm -rf /var/lib/dkms/nvidia* /usr/src/nvidia* /etc/modprobe.d/nvidia* /etc/modprobe.d/glx* /usr/lib/nvidia 2>/dev/null || true",
      ...
      "pct exec ${var.container_vm_id} -- bash -c \"export PATH=\\$PATH:/usr/local/bin:/usr/sbin:/sbin; rm -f /etc/apt/sources.list.d/debian-gpu.list 2>/dev/null || true; if [ -f /etc/apt/sources.list.d/debian.sources ]; then sed -i -E 's/Components: main( contrib)?( non-free)?( non-free-firmware)?$/Components: main contrib non-free non-free-firmware/g' /etc/apt/sources.list.d/debian.sources; fi; if [ -f /etc/apt/sources.list ]; then sed -i -E 's/(main|main contrib)$/\\1 contrib non-free non-free-firmware/' /etc/apt/sources.list || true; fi; apt-get update -y || true; DEBIAN_FRONTEND=noninteractive apt-get install -y nvidia-smi nvidia-driver-libs || true; /usr/local/bin/k3s kubectl rollout restart daemonset nvidia-device-plugin-daemonset -n kube-system || true; /usr/local/bin/k3s kubectl rollout restart daemonset nvidia-dcgm-exporter -n kube-system || true\"",
