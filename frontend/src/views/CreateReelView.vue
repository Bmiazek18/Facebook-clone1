<template>
  <div class="fixed inset-0 bg-black z-50 flex flex-col">
    <div
      class="bg-gray-900 text-white px-4 py-3 flex items-center justify-between border-b border-gray-700"
    >
      <button
        @click="$emit('back')"
      ...
      >
        <span
          v-if="isRendering"
          class="animate-spin w-4 h-4 border-2 border-white rounded-full border-t-transparent"
        ></span>
    ...
    <div class="flex-1 flex overflow-hidden">
      <div class="flex-1 flex items-center justify-center bg-gray-950 p-4">
        <div
          class="relative max-w-[360px] w-full aspect-9/16 bg-black rounded-lg overflow-hidden shadow-2xl"
        >
          <video
            ref="previewVideoRef"
      ...
      <div class="w-80 bg-gray-900 border-l border-gray-700 overflow-y-auto">
        <div class="p-4 space-y-4">
          ...
          <PipVideoEditor
            :video="selectedPipVideo"
            @update="updateSelectedPipVideo"
            @delete="deletePipVideo"
          />
        </div>
      </div>
    </div>
    ...
    <div class="bg-gray-900 border-t border-gray-700 p-4">
      <div class="max-w-full">
        ...
        <div
          ref="timelineRef"
          class="relative bg-gray-800 rounded-lg p-3 overflow-x-auto"
            ...
            <div class="relative h-16 bg-gray-700 rounded flex">
              <template v-for="(clip, index) in clips" :key="clip.id">
                ...
                >
                  ...
                  <div
                    class="absolute inset-0 bg-blue-500/20 opacity-0 group-hover:opacity-100 transition-opacity"
                  ></div>
                  ...
                  <div
                    class="absolute bottom-1 left-1 text-white text-[10px] bg-black/70 px-1 rounded"
                  >
...
// --- Sub-components ---
import TextEditor from '@/components/create/createReel/TextEditor.vue'
import ImageEditor from '@/components/create/createReel/ImageEditor.vue'
import PipVideoEditor from '@/components/create/createReel/PipVideoEditor.vue'
import TimelineTrack from '@/components/create/createReel/TimelineTrack.vue'

// --- Composables ---
import { useVideoExport } from '@/composables/media/useVideoExport'
import { usePreviewStyles } from '@/composables/ui/usePreviewStyles'
...
const visibleTexts = computed(() => {
  return textOverlays.value.filter(
    (text) => currentTime.value >= text.startTime && currentTime.value <= text.endTime,
  )
})
...
const visibleImages = computed(() => {
  return imageOverlays.value.filter(
    (image) => currentTime.value >= image.startTime && currentTime.value <= image.endTime,
  )
})

const visiblePipVideos = computed(() => {
  return pipVideoOverlays.value.filter(
    (video) => currentTime.value >= video.startTime && currentTime.value <= video.endTime,
  )
})
...
const { getTextStyle, getTextContent, getImageStyle, getPipVideoStyle } =
  usePreviewStyles(currentTime)
...
const deletePipVideo = () => {
  if (selectedPipVideo.value) {
    pipVideoOverlays.value = pipVideoOverlays.value.filter(
      (v) => v.id !== selectedPipVideo.value!.id,
    )
    selectedPipVideo.value = null
  }
}
...
const removeClip = (index: number) => {
  ...
  if (currentClipIndex.value >= clips.value.length)
    currentClipIndex.value = Math.max(0, clips.value.length - 1)
}
...
const startAnimationLoop = () => {
  const updateFrame = () => {
    if (!isPlaying.value || !previewVideoRef.value) {
      animationFrameId = null
      return
    }
