...
<template>
  <BaseModal v-if="isOpen" :title="$t('chat.polaczeniePrzychodzace')" @close="handleClose">
    <div class="w-[300px] flex flex-col items-center p-4">
      ...
      <p class="text-xl font-bold text-theme-text text-center mb-2">{{ $t('chat.dzwoniDoCiebie') }}</p>

      <div class="flex items-center gap-1.5 text-xs text-gray-500 font-medium mb-8">
        <LockIcon :size="16" fillColor="currentColor" />{{ $t('chat.pelneSzyfrowanie') }}</div>
      ...
      <div class="flex items-center justify-between w-full px-4 mb-2">
        <div class="flex flex-col items-center gap-2">
          ...
          <span class="text-xs font-semibold text-theme-text">{{ $t('chat.odrzuc') }}</span>
        </div>
        ...
        <div class="flex flex-col items-center gap-2">
          ...
          <span class="text-xs font-semibold text-theme-text">{{ $t('chat.zaakceptuj') }}</span>
        </div>
      </div>
    </div>
  </BaseModal>
