<template>
  <div
    class="bg-theme-bg-secondary w-[800px] rounded-xl shadow-[0_12px_28px_rgba(0,0,0,0.2)] flex flex-col overflow-hidden relative border border-theme-border z-10 h-[850px]"
  >
    <BaseModal :title="locationModalTitle" v-if="showLocationModal">
      <LocationModal
    ...
    <div class="overflow-y-auto flex-1 custom-scrollbar relative" @click="activeDatePicker = null">
      <div class="relative w-full h-[200px] bg-theme-bg group shrink-0">
        ...
        <button
          ...
          class="absolute bottom-4 right-4 bg-black/40 hover:bg-black/50 text-white px-4 py-2 rounded-lg font-semibold text-[15px] flex items-center gap-2 transition backdrop-blur-sm z-10"
        >
        ...
        <button
          ...
          class="absolute top-1/2 left-2 -translate-y-1/2 bg-black/40 hover:bg-black/50 text-white p-2 rounded-full backdrop-blur-sm transition z-10"
        >
        ...
        <button
          ...
          class="absolute top-1/2 right-2 -translate-y-1/2 bg-black/40 hover:bg-black/50 text-white p-2 rounded-full backdrop-blur-sm transition z-10"
        >
      ...
      <div class="p-4">
        ...
        <div class="mb-4 relative group">
          <input
            ...
            id="eventName"
            maxlength="100"
            class="block rounded-lg pl-4 pr-16 pt-6 pb-2 w-full text-[17px] text-theme-text bg-transparent border border-theme-border appearance-none focus:outline-none focus:ring-0 focus:border-theme-primary peer"
            placeholder=" "
          />
          <label
            for="eventName"
        ...
        <div class="space-y-4 mb-4">
          <div class="grid grid-cols-2 gap-4">
            <div class="relative">
              <div
                ...
                :class="[
                  'border rounded-lg p-2 cursor-pointer flex items-center gap-2 hover:bg-theme-hover transition',
                  activeDatePicker === 'start' ? 'border-theme-primary' : 'border-theme-border',
                ]"
              >
              ...
              <div
                ...
                class="absolute top-full left-0 z-50 mt-1 shadow-xl rounded-xl"
                @click.stop
              >
                <VDatePicker
                  ...
                  mode="date"
                  locale="pl"
                  @dayclick="activeDatePicker = null"
                />
              </div>
            </div>
            <div
              class="border border-theme-border rounded-lg p-2 hover:bg-theme-hover cursor-pointer flex items-center gap-2 relative overflow-hidden"
            >
              ...
              <div class="flex flex-col w-full">
                ...
                ><input
                  ...
                  type="time"
                  class="text-[15px] text-theme-text bg-transparent border-none p-0 focus:ring-0 w-full  "
                />
              </div>
            </div>
          </div>
          ...
          <div
            v-if="showEndDate"
            class="grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 duration-200"
          >
            <div class="relative">
              <div
                ...
                :class="[
                  'border rounded-lg p-2 cursor-pointer flex items-center gap-2 hover:bg-theme-hover transition',
                  activeDatePicker === 'end' ? 'border-theme-primary' : 'border-theme-border',
                ]"
              >
                <calendar-month-icon class="text-theme-text-secondary" />
                <div class="flex flex-col">
        ...
        <div v-if="form.type === 'offline'" class="mb-4 relative">
          <div
            ...
            :class="{
              'text-theme-text-secondary': !form.location,
              'text-theme-text': form.location,
            }"
          >
            {{ form.location || 'Dodaj lokalizację' }}
          </div>
          <div
            class="absolute right-1.5 top-1.5 bottom-1.5 aspect-square bg-theme-bg-subtle hover:bg-theme-hover-strong rounded-md flex items-center justify-center cursor-pointer transition pointer-events-none"
          >
          ...
          <Dropdown class="w-full" :distance="6">
            ...
            <template #popper>
              <div class="w-[340px] p-2 bg-theme-bg-secondary rounded-xl shadow-xl">
                ...
                >
                  <div
                    class="bg-theme-bg-subtle p-2.5 rounded-full text-theme-text flex items-center justify-center"
                  >
                    <component :is="option.icon" :size="24" />
                  </div>
                  <div class="flex flex-col flex-1">
                    <span class="text-[15px] font-bold text-theme-text">{{ option.title }}</span>
                    <span class="text-[13px] text-theme-text-secondary leading-tight">{{
                      option.desc
                    }}</span>
                  </div>
                  <div
                    v-if="form.privacy === option.id"
                    class="w-5 h-5 rounded-full border-[6px] border-theme-primary bg-theme-bg-secondary"
                  ></div>
                  <div v-else class="w-5 h-5 rounded-full border border-theme-text-secondary"></div>
                  <chevron-right-icon
                    v-if="option.hasArrow"
                    class="absolute right-2 text-theme-text-secondary"
                    :size="24"
                  />
                </div>
              </div>
        ...
        <div class="space-y-4">
          <div class="border-t border-theme-border">
            <button
              @click.stop="toggleAccordion('cohosts')"
              class="flex items-center justify-between w-full py-4 hover:bg-theme-hover -mx-4 px-4 transition"
            >
              ...
              <chevron-down-icon
                :class="[
                  'text-theme-text-secondary transition-transform',
                  accordions.cohosts ? 'rotate-180' : '',
                ]"
                :size="20"
              />
            </button>
          ...
          <div class="border-t border-theme-border">
            <button
              @click.stop="toggleAccordion('recurring')"
              class="flex items-center justify-between w-full py-4 hover:bg-theme-hover -mx-4 px-4 transition"
            >
              <div class="flex items-center gap-3">
              ...
              <chevron-down-icon
                :class="[
                  'text-theme-text-secondary transition-transform',
                  accordions.recurring ? 'rotate-180' : '',
                ]"
                :size="20"
              />
            </button>
          ...
          <div class="border-t border-theme-border">
            <button
              @click.stop="toggleAccordion('settings')"
              class="flex items-center justify-between w-full py-4 hover:bg-theme-hover -mx-4 px-4 transition"
            >
              <div class="flex items-center gap-3">
              ...
              <chevron-down-icon
                :class="[
                  'text-theme-text-secondary transition-transform',
                  accordions.settings ? 'rotate-180' : '',
                ]"
                :size="20"
              />
            </button>
    ...
    <div
      class="p-4 border-t border-theme-border bg-theme-bg-secondary shadow-[0_-4px_12px_rgba(0,0,0,0.05)] shrink-0 z-20"
    >
      <button
        @click="submit"
...
const eventTypeOptions = [
  {
    id: 'offline',
    title: 'Offline',
    description: 'Spotkajcie się w określonej lokalizacji.',
    icon: SendOutlineIcon,
  },
  {
    id: 'online',
    title: 'Wirtualne',
    description: 'Odbądźcie spotkanie wideo lub transmisję na żywo.',
...
const selectedPrivacyOption = computed(() =>
  privacyOptions.find((opt) => opt.id === form.value.privacy),
)
...
watch(
  ...
  (newStart) => {
    if (newStart > form.value.endDate) {
