...
const templates = [
  {
    id: 'spam_comments',
    title: 'Spam comments',
    desc: 'Zapobiegaj pojawianiu się niechcianych komentarzy promocyjnych i innych form spamu w grupie.',
    icon: markRaw(CommentRemoveOutlineIcon),
    iconBg: 'bg-[#ff5a5f]',
    criteriaCount: 4,
    rules: [
      'Komentarz zawiera pewne słowa kluczowe',
      'Autor nie ma zdjęcia profilowego.',
      'Komentarz zawiera linki do określonych witryn',
      `Autor ma konto na Facebooku od co najwyżej ${p('1 miesiąca')} .`
    ]
  },
  {
    id: 'rule_breakers',
    title: 'Komentarze osób, którym zdarzyło się naruszyć zasady',
    desc: 'Możesz uniemożliwić komentowanie osobom, które niedawno naruszyły zasady grupy lub których publikacje zostały zgłoszone.',
    icon: markRaw(SyncIcon),
    iconBg: 'bg-[#00a48d]',
    criteriaCount: 2,
    rules: [
      ...
      >
        ...
        <div v-else-if="currentView === 'detail' && selectedTemplate" key="detail" class="view-container flex flex-col max-h-[85vh]">
          ...
          <div class="p-4 sm:p-6 pt-5 overflow-y-auto flex-1">
            ...
            <!-- Lista Reguł -->
            <div>
              <h3 class="text-[20px] font-bold mb-4">{{ $t('groups.odrzucNadchodzacyKomentarzJesli') }}</h3>
