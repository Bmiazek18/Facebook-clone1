...
const templates = [
  {
    id: 'conflict',
    title: 'Posty z konfliktem',
    desc: 'Automatycznie podejmij działania zapobiegawcze w przypadku oznak konfliktu.',
    icon: markRaw(CommentRemoveOutlineIcon),
    iconBg: 'bg-[#e4405f]',
    criteriaCount: 2,
    categories: [
      {
        title: 'Wyłącz komentowanie, jeśli',
        rules: [
          `Post uzyskał co najmniej ${p(100)} komentarzy w ciągu ostatniej godziny`,
          `Post został zgłoszony co najmniej ${p(3)} razy`
        ]
      }
    ]
  },
  {
    id: 'controversial',
    title: 'Kontrowersyjne posty',
    desc: 'Automatycznie podejmij działanie w przypadku postów, które zostały zgłoszone lub mogą być kontrowersyjne.',
    icon: markRaw(FireIcon),
    iconBg: 'bg-[#f57c00]',
    criteriaCount: 3,
    categories: [
      {
        title: 'Usuń opublikowany post, jeśli',
        rules: [
          `Post został zgłoszony co najmniej ${p(3)} razy`
        ]
      },
  ...
  {
    id: 'harmful',
    title: 'Posty ze szkodliwą zawartością',
    desc: 'Automatycznie reaguj na potencjalnie problematyczne materiały pojawiające się w grupie.',
    icon: markRaw(ShieldOutlineIcon),
    iconBg: 'bg-[#ff5a5f]',
    criteriaCount: 3,
    categories: [
      {
      ...
      >
        ...
        <div v-else-if="currentView === 'detail' && selectedTemplate" key="detail" class="view-container flex flex-col max-h-[85vh]">
          ...
          <div class="p-4 sm:p-6 pt-5 overflow-y-auto flex-1">
            ...
            <!-- Listy reguł pogrupowane w kategorie -->
            <div class="flex flex-col gap-8">
              <div
                v-for="(category, catIndex) in selectedTemplate.categories"
                :key="catIndex"
              >
                <h3 class="text-[20px] font-bold mb-4">{{ category.title }}</h3>
