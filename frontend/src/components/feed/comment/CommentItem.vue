...
import { useCommentReactions } from '@/composables/feed/useCommentReactions'
...
const startReply = () => {
  ...
  emit('reply', {
    author: { id: props.comment.authorId, name: authorName.value },
...
const viewCommentImage = (commentId: number, postId: string) => {
  router.push(`/comment/${postId}/${commentId}`)
}
...
const commentWrapperClass = computed(() => {
  return {
    'w-fit p-2 rounded-xl': props.comment.content.length > 0,
    ...
    'bg-theme-comment-bg w-fit px-3 rounded-xl':
      !isCommentActiveForReply.value && props.comment.content.length > 0,
  }
})
</script>
<template>
  <div :class="{ 'mt-4': isRootComment, 'mt-2': !isRootComment }">
    <div class="flex" :style="{ marginLeft: isRootComment ? '0px' : `-32px` }">
      <!-- Linia łącząca dla odpowiedzi -->
      <div v-if="!isRootComment" class="w-8 mr-2 relative flex-shrink-0">
        <div
          class="absolute h-4 border-b-2 border-l-2 border-theme-secondary right-0 rounded-bl-[10px]"
          :class="props.depth > 1 ? 'w-[21px]' : 'w-[25px]'"
        ></div>
      </div>
      ...
      <div class="flex-grow ml-2">
        <div :class="commentWrapperClass">
          <div class="flex flex-col w-fit">
            ...
            <ProfilePopper :userId="author?.id" :name="authorName" mention class="font-semibold" />
          </div>
        ...
        <CommentLinkPreview
          v-if="props.comment.linkPreview"
          :link-preview="props.comment.linkPreview"
        />
        <!-- Akcje komentarza -->
        <div
          class="flex items-center space-x-2 mt-1 font-semibold text-[12px] text-theme-text-secondary"
        >
          ...
          <span @click="startReply" class="cursor-pointer hover:underline">
            {{ $t('actions.reply') }}
          </span>
          <CommentReactions
            :comment="comment"
        ...
        <div
          v-if="hasReplies && !showReplies && props.depth <= 2"
          class="flex mt-2 items-center"
          :class="isRootComment ? '-ml-[36px]' : '-ml-[32px]'"
        >
          <div class="w-8 mr-2 relative">
            <div
              class="absolute w-[21px] -mt-5 h-5 border-b-2 border-l-2 border-theme-secondary right-0 rounded-bl-[10px]"
            ></div>
          </div>
          <button
        ...
        <div
          v-if="isReplying && !showReplies && props.depth < 2"
          class="flex mt-2 items-start"
          :class="showReplies || props.depth == 0 ? 'ml-[-30px]' : 'ml-[-26px]'"
        >
          <div class="w-8 mr-2 relative">
            <div
              class="absolute h-5 border-b-2 border-l-2 border-theme-secondary bg-white right-0 rounded-bl-[10px]"
              :class="props.depth > 1 ? 'w-[21px]' : 'w-[25px]'"
            ></div>
          </div>
          <CommentReplyInput
        ...
        <div
          v-if="showReplies && props.comment.replies && props.comment.replies.length > 0"
          class="mt-2"
        >
          <CommentItem
          ...
          <div
            class="flex mt-2 items-start relative bg-theme-bg-secondary"
            :class="props.depth < 2 ? 'ml-[-30px]' : 'ml-[-26px]'"
          >
            <div class="w-8 mr-2 relative">
              <div
                class="absolute h-5 border-b-2 border-l-2 border-theme-secondary right-0 rounded-bl-[10px]"
                :class="props.depth == 1 ? 'w-[21px]' : 'w-[25px]'"
              ></div>
            </div>
            <CommentReplyInput
