<template>
  <div class="mx-3 mb-3 mt-2 rounded-lg overflow-hidden" v-if="likesCount > 0 || commentsCount > 0 || sharesCount > 0">
    <div class="flex items-center justify-between text-theme-text-secondary text-[15px]">
      ...
      <!-- LEWA STRONA: Liczba polubień oraz ewentualne komentarze / udostępnienia -->
      <div class="flex items-center gap-4">
        <!-- Licznik reakcji (po lewej stronie) -->
        <div
          v-if="likesCount > 0"
          class="flex items-center cursor-pointer group"
          @click="emit('show-reaction-details')"
        >
          <span class="text-theme-text-secondary group-hover:underline leading-snug font-medium">
        ...
        <!-- Komentarze i udostępnienia (jeśli mają być obok licznika po lewej) -->
        <div v-if="commentsCount > 0 || sharesCount > 0" class="flex items-center gap-3 text-theme-text-secondary text-[14px]">
          <span
      ...
      <!-- PRAWA STRONA: Nakładające się ikonki reakcji (kolorowe kółka) -->
      <div
        v-if="likesCount > 0"
            ...
            <VTooltip>
              ...
              <template #popper>
                <div class="flex flex-col text-[13px] rounded-md">
                  ...
                  >
                    {{ $t('feed.iGetreactiontooltipdataReactiontypeAs') }}
                  </span>
                </div>
              </template>
            </VTooltip>
