      ...
      <div class="w-full flex items-end justify-between gap-3 max-w-[650px] px-4 md:px-0">
        <div class="relative flex-1 h-[44px]">
          <input
            ...
            type="text"
            :placeholder="
              currentItem.type === 'birthday' ? 'Złóż życzenia' : t('story.sendMessage')
            "
        ...
        <div
          v-if="currentItem.type !== 'birthday'"
          class="flex items-center gap-2 pb-0.5 hidden sm:flex"
        >
          <div
            v-for="(reaction, idx) in reactions"
            :key="idx"
            class="cursor-pointer hover:scale-125 active:scale-95 transition-transform duration-200 origin-bottom"
          >
            <div
              v-if="reaction.type === 'icon'"
              class="w-[38px] h-[38px] rounded-full flex items-center justify-center shadow-lg border-2 border-transparent"
            ...
            <div
              v-else
              class="text-[36px] leading-none drop-shadow-md filter hover:brightness-110 select-none"
            >
              {{ reaction.content }}
            </div>
          </div>
