  ...
  >
    <div class="flex-1 flex flex-col gap-1 p-2 pt-4 overflow-y-auto">
      <template v-for="tool in tools" :key="tool.id">
        ...
        >
          <div
            class="w-9 h-9 rounded-full bg-theme-bg-tertiary flex items-center justify-center text-theme-text group-hover:bg-theme-hover transition-colors shrink-0"
          >
    ...
    <div
      class="p-4 flex items-center gap-3 border-t border-theme-border bg-theme-bg-secondary shadow-[0_-2px_4px_rgba(0,0,0,0.05)]"
    >
      <template v-if="!isCroppingMode">
        <button
          @click="emit('done')"
