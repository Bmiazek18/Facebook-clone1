...
<template>
  <div class="w-full h-full relative select-none">
    ...
    <template v-if="element.musicStyle === 'large' || !element.musicStyle">
      <div
        class="w-full h-full bg-white/95 backdrop-blur-sm rounded-xl p-3 flex flex-col gap-3 pointer-events-none border border-gray-100"
      >
        <img
          :src="element.content"
        ...
        <div class="text-center overflow-hidden pb-1">
          <p class="text-gray-900 font-bold text-lg leading-tight truncate">
            {{ element.musicTitle }}
          </p>
    ...
    <template v-else-if="element.musicStyle === 'small'">
      <div
        class="w-full h-full bg-white/90 backdrop-blur-md rounded-lg p-2 flex items-center gap-3 pointer-events-none border border-white/20"
      >
    ...
    <template v-else-if="element.musicStyle === 'text'">
      <div
        class="w-full h-full flex flex-col items-center justify-center pointer-events-none text-center"
      >
        <p class="text-white font-bold text-2xl drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)]">
          {{ element.musicTitle }}
        </p>
        <div
          class="flex items-center gap-1 opacity-90 drop-shadow-[0_1px_4px_rgba(0,0,0,0.6)] mt-1"
        >
    ...
    <template v-else-if="element.musicStyle === 'icon'">
      <div
        class="w-full h-full rounded-full shadow-lg border-2 border-white pointer-events-none relative overflow-hidden bg-black"
      >
        <img
          :src="element.content"
