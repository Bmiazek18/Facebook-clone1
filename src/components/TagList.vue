...
<template>
  <div class="mt-4 p-2">
    ...
    <div class="border border-gray-300 rounded-md p-2 min-h-[40px] flex flex-nowrap gap-2 overflow-x-auto">
      <div
        v-for="tag in tags"
        :key="tag.id"
        class="flex items-center bg-blue-50 text-blue-600 px-2 py-1.5 rounded-[4px] cursor-default"
      >
        <span class="font-semibold text-sm mr-1">{{ tag.name }}</span>
        <button @click="handleRemove(tag.id)" class="hover:text-blue-800 flex items-center">
          <CloseIcon :size="16" />
        </button>
      </div>
    </div>
