    ...
    <Dropdown class="w-full" :distance="4" :disabled="disabled" placement="bottom-start">
      ...
      <template #popper="{ hide }">
        <div :class="popperClasses" :style="{ width: dropdownWidth }">
          ...
          >
            <div v-if="option.icon" :class="['p-2 rounded-full flex items-center justify-center', modelValue === option.id ? 'bg-white/20' : 'bg-[#f5f6f7]']">
...
const triggerClasses = computed(() => {
  ...
  const disabledClass = props.disabled ? 'opacity-50 cursor-not-allowed bg-[#f5f6f7]' : 'cursor-pointer';

  if (props.variant === 'new') {
    const shape = 'px-4 pb-2.5 pt-6 border rounded-xl bg-transparent hover:bg-gray-50';
    const state = props.error ? 'border-[#d32f2f] ring-1 ring-[#d32f2f]' : 'border-[#ccd0d5] focus:border-[#1877f2] focus:ring-1 focus:ring-[#1877f2]';
    return [base, disabledClass, shape, state];
  } else {
    const shape = 'px-3 pb-2 pt-6 border rounded-[6px] bg-[#f5f6f7] hover:bg-white';
    const state = props.error ? 'border-[#b0281c] ring-1 ring-[#b0281c]/20' : 'border-[#ccd0d5] focus:border-[#1877f2]';
    return [base, disabledClass, shape, state];
  }
});
...
const popperClasses = computed(() => {
  ...
  const base = 'bg-white shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-gray-100 max-h-[350px] overflow-y-auto';
