<template>
  <div class="relative w-full">
    ...
    <div
      :class="[
        'absolute top-1/2 -translate-y-1/2 flex items-center gap-2 text-[#606770]',
        variant === 'new' ? 'right-4' : 'right-3',
      ]"
    >
      <slot name="icon"></slot>
    </div>
...
const inputClasses = computed(() => {
  const base =
    'peer block w-full border text-[15px] text-[#1c1e21] focus:outline-none transition-all placeholder-transparent'
...
const labelClasses = computed(() => {
  ...
  const base =
    'absolute z-10 origin-[0] transform duration-300 cursor-text peer-focus:top-1 peer-focus:-translate-y-0 peer-focus:scale-75'
  ...
  const labelFocusColor = props.disableFocusColor
    ? 'peer-focus:text-[#606770]'
    : 'peer-focus:text-[#1877f2]'

  if (props.variant === 'new') {
    const pos = 'left-4'
    const colors = props.error
      ? 'text-[#d32f2f] peer-focus:text-[#d32f2f]'
      : `text-[#606770] ${labelFocusColor}`
    return [base, position, pos, colors]
  } else {
    // Wariant Classic
    const pos = 'left-3'
    const colors = props.error
      ? 'text-[#b0281c] peer-focus:text-[#b0281c]'
      : `text-[#606770] ${labelFocusColor}`
    return [base, position, pos, colors]
  }
})
