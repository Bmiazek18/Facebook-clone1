<template>
  <div>
      ...
      <div class="options-list space-y-2">
        ...
        >
          ...
          <div class="relative z-10 flex items-center p-3.5 w-full">
            ...
            >
              <svg v-if="isSelected(option)" xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <span
              ...
              :class="[
                 hasVoted && isSelected(option) ? 'text-gray-900 dark:text-white font-semibold' : 'text-gray-800 dark:text-gray-200'
              ]"
            >
              {{ option.text }}
            </span>

            <div v-if="hasVoted" class="ml-auto flex items-center gap-1 pl-2"
    ...
    <BaseModal v-if="isVotersModalOpen" @close="closeVotersModal" :title="`Użytkownicy, którzy wybrali: ${selectedOptionText}`">
      <div class="p-4">
        <ul class="space-y-2">
          <li v-for="voterId in selectedOptionVoters" :key="voterId" class="flex items-center text-theme-text">
