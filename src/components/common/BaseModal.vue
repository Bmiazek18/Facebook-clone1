...
<template>
  <Teleport to="body">
    ...
    >
      ...
      >
        <div v-if="!noHeader" class="p-3 border-b border-black/10 grid grid-cols-[auto_1fr_auto] items-center gap-4">
          <div class="flex justify-start min-w-10">
            <button
              v-if="back"
              @click="handleBack"
              class="text-theme-text bg-[#dee1e6] hover:bg-[#d3d6da] rounded-full p-2 transition-colors cursor-pointer"
            >
              <ArrowLeft :size="24" class="text-gray-500" />
            </button>
          </div>
          ...
          <div class="flex justify-end min-w-10">
            <button
              @click="closeModal"
              class="text-theme-text bg-[#dee1e6] hover:bg-[#d3d6da] rounded-full p-1.5 cursor-pointer"
            >
