...
const isVideo = computed(() => {
  return props.media.src.toLowerCase().endsWith('.mp4') || props.media.src.toLowerCase().endsWith('.webm')
})
...
<template>
  ...
  >
    <!-- Zoom controls -->
    <div v-if="!isVideo" class="absolute top-4 right-4 flex space-x-2 z-40">
      <button
        @click="zoomIn"
    ...
    >
      ...
      />
      <template v-if="!isVideo && props.media.tags && props.media.tags.length > 0">
        <ImageTag
