...
const viewersWithReactions = computed(() => {
  return props.interactions
    ...
    .filter(
      (item): item is { viewer: User; reaction: ReactionType | null } => !!item.viewer
    )
})
...
<template>
  <div v-if="show" class="absolute inset-0 z-50 flex flex-col items-center justify-end bg-black/60 backdrop-blur-sm" @click.self="close">
