...
<template>
   <div class="flex items-center justify-end w-[260px] relative">
      ...
      <button @click="toggleMenu('message')" v-tooltip.bottom.no-arrow="'Wiadomości'" class="rounded-full bg-[#E3E6EA] dark:bg-[#3b3d3f] p-2 hover:bg-gray-300 dark:hover:bg-gray-600 mx-1">
        <FacebookMessenger :size="23" :fillColor="isDark ? '#fff' : '#050505'" />
      </button>

      <button @click="toggleMenu('notifications')" v-tooltip.bottom.no-arrow="'Powiadomienia'" class="relative rounded-full bg-[#E3E6EA] dark:bg-[#3b3d3f] p-2 hover:bg-gray-300 dark:hover:bg-gray-600 mx-1">
      ...
      <div class="flex items-center relative">
        <button @click="toggleMenu('profile')" v-tooltip.bottom.no-arrow="'Konto'" class="relative">
          ...
          <div class="absolute bottom-0 right-0 bg-[#E3E6EA] dark:bg-[#3b3d3f] rounded-full p-px border-2 border-white dark:border-[#242526]">
