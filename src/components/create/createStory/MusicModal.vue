...
<template>
  <Transition name="pop">
    <div
      v-if="isOpen"
      class="absolute -right-[115px] top-15 w-[320px] bg-theme-bg-secondary rounded-xl shadow-2xl z-300 flex flex-col h-[500px] overflow-hidden border border-theme-border"
    >
      <div class="p-3 border-b border-theme-border">
        ...
        <div class="relative">
          ...
          <button
            ...
            @click="
              ...
              musicResults = [];
            "
            class="absolute right-3 top-2.5 text-theme-text-secondary hover:text-theme-text"
          >
            <Close :size="16" />
          </button>
        </div>
        <div class="flex mt-3 gap-2">
          <button
            ...
            :class="
              currentTab === 'saved'
                ? 'bg-theme-text text-theme-bg-secondary'
      ...
      <HoverScrollbar class="flex-1 overflow-y-auto custom-scrollbar">
        <div class="px-2 py-2">
          ...
          <div
            ...
            :key="track.id"
            class="group flex items-center justify-between py-2 px-2 hover:bg-theme-hover rounded-lg transition cursor-pointer"
          ...
          >
            <div class="flex items-center gap-3 overflow-hidden">
              <div class="relative w-10 h-10 shrink-0 rounded overflow-hidden bg-theme-bg">
                ...
                >
                  <button
                    @click.stop="togglePreviewTrack(track)"
                    class="text-white hover:scale-110 transition"
                  >
                    <Pause
                      v-if="currentPreviewTrackId === track.id && isPreviewPlaying"
                      :size="18"
                    /><Play v-else :size="18" />
                  </button>
                </div>
            ...
            <div class="flex items-center gap-1">
              <button
                @click.stop="toggleSaveMusic(track)"
                class="p-1 text-theme-text-secondary hover:text-blue-600 rounded-full hover:bg-blue-50 transition"
                :title="isSaved(track.id) ? 'Usuń z ulubionych' : 'Zapisz'"
              >
          ...
          <div v-if="!displayedMusic.length && !isSearching" class="text-center py-8">
            ...
            <p class="text-theme-text-secondary text-xs">
              {{
                currentTab === 'saved' ? 'Brak zapisanych utworów.' : 'Wpisz nazwę utworu powyżej.'
              }}
            </p>
          </div>
        </div>
