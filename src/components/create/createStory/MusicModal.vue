...
<template>
  <Transition name="pop">
     <div v-if="isOpen" class="absolute -right-[115px] top-15 w-[320px] bg-theme-bg-secondary rounded-xl shadow-2xl z-300 flex flex-col h-[500px] overflow-hidden border border-theme-border">
         <div class="p-3 border-b border-theme-border">
            <div class="flex justify-between items-center mb-2">
             ...
             <div class="relative">
                 <Magnify class="absolute left-3 top-2.5 text-theme-text-secondary" :size="20"/>
                 <input v-model="musicQuery" @keydown.enter="searchMusic" type="text" placeholder="Szukaj muzyki" class="w-full pl-10 pr-8 py-2 bg-theme-bg rounded-full text-sm placeholder:text-theme-text-secondary focus:outline-none focus:ring-1 focus:ring-blue-500 transition"/>
                 <button v-if="musicQuery" @click="musicQuery = ''; musicResults = []" class="absolute right-3 top-2.5 text-theme-text-secondary hover:text-theme-text"><Close :size="16"/></button>
             </div>
             <div class="flex mt-3 gap-2">
                 <button @click="currentTab = 'saved'" class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition" :class="currentTab === 'saved' ? 'bg-theme-text text-theme-bg-secondary' : 'hover:bg-theme-hover text-theme-text-secondary'"><Bookmark :size="14" />Zapisane</button>
                 <button @click="currentTab = 'browse'" class="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition" :class="currentTab === 'browse' ? 'bg-theme-text text-theme-bg-secondary' : 'hover:bg-theme-hover text-theme-text-secondary'"><FormatListBulleted :size="14" />Przeglądaj</button>
             </div>
         </div>

         <HoverScrollbar class="flex-1 overflow-y-auto custom-scrollbar">
             <div class="px-2 py-2">
                 <h3 class="text-xs font-bold text-theme-text-secondary mb-2 px-2 uppercase tracking-wide">{{ currentTab === 'saved' ? 'Twoje ulubione' : 'Dla Ciebie' }}</h3>
                 <div v-if="isSearching" class="text-center py-4 text-theme-text-secondary text-sm">Szukam...</div>

                 <div v-for="track in displayedMusic" :key="track.id" class="group flex items-center justify-between py-2 px-2 hover:bg-theme-hover rounded-lg transition cursor-pointer" @click="selectTrack(track)">
                     <div class="flex items-center gap-3 overflow-hidden">
                         <div class="relative w-10 h-10 shrink-0 rounded overflow-hidden bg-theme-bg">
                         ...
                         <div class="flex flex-col min-w-0">
                             <span class="text-sm font-semibold text-theme-text truncate">{{ track.title }}</span>
                             <span class="text-xs text-theme-text-secondary truncate">{{ track.artist }}</span>
                         </div>
                     </div>
                     <div class="flex items-center gap-1">
                         <button @click.stop="toggleSaveMusic(track)" class="p-1 text-theme-text-secondary hover:text-blue-600 rounded-full hover:bg-blue-50 transition" :title="isSaved(track.id) ? 'Usuń z ulubionych' : 'Zapisz'">
                 ...
                 <div v-if="!displayedMusic.length && !isSearching" class="text-center py-8">
                     <MusicNote class="mx-auto text-theme-text-secondary mb-2" :size="32"/>
                     <p class="text-theme-text-secondary text-xs">{{ currentTab === 'saved' ? 'Brak zapisanych utworów.' : 'Wpisz nazwę utworu powyżej.' }}</p>
                 </div>
             </div>
         </HoverScrollbar>
     </div>
