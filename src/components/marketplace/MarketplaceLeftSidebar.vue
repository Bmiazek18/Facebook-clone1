...
<template>
  ...
  <template #settings-dropdown>
      <div class="w-[360px] max-h-[80vh] overflow-y-auto bg-theme-surface rounded-2xl p-4 text-theme-text shadow-2xl border border-theme-border">
        <h2 class="text-xl font-bold mb-3">Ustawienia Marketplace</h2>
        <!-- Dowolne przełączniki / przyciski / komponenty -->
      </div>
    </template>
    <template #pre-list>
      ...
      <div>
        ...
        <ul class="space-y-1">
          <li v-for="(cat, index) in categories" :key="index">
            <a
              href="#"
              class="flex items-center px-2 py-2 hover:bg-theme-hover rounded-lg cursor-pointer"
            >
              <div class="mr-3">
