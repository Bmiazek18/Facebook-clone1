...
import { reactionIcons } from '@/composables/usePostReactions'; // Assuming these are reusable
...
<template>
  <div v-if="show" class="absolute inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" @click.self="emit('close')">
    <div class="bg-white dark:bg-[#242526] w-full max-w-md rounded-lg shadow-xl flex flex-col max-h-[80vh]">
        ...
        <div v-else>
          <div v-for="{ reaction, users } in reactionsByType" :key="reaction" class="mb-4">
            <div class="flex items-center mb-2">
              ...
              <h4 class="font-semibold text-lg dark:text-white">{{ reaction.charAt(0).toUpperCase() + reaction.slice(1) }}</h4>
            </div>
            <ul>
              <li v-for="user in users" :key="user.id" class="flex items-center gap-3 p-2 rounded-lg">
