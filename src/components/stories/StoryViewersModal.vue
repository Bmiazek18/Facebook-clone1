...
const emit = defineEmits(['update:showViewers', 'update:isPaused']);

const close = () => {
  emit('update:showViewers', false);
  emit('update:isPaused', false);
};
</script>

<template>
  <div v-if="showViewers" class="absolute inset-0 z-50 flex flex-col items-center justify-end bg-black/60 backdrop-blur-sm" @click.self="close">
