  ...
  <div class="space-y-8 text-base">
    ...
    <div v-if="hasUniversityInfo || isOwner">
      ...
      <div v-if="profileUser.school || profileUser.education" class="flex justify-between items-center mb-4">
         ...
         <div v-if="isOwner" class="flex items-center space-x-2 text-gray-500">
            <Earth class="text-lg"/>
            <ItemMenu editText="Edytuj szkołę wyższą" removeText="Usuń szkołę wyższą" @edit="editUniversity" @remove="remove('uni')"/>
         </div>
      </div>
      <div class="mt-2">
        <EditForm v-if="activeSection === 'uni'" label="Uczelnia" v-model="form.university" @cancel="close" @save="save('uni')" />
    ...
    <div v-if="hasHighSchoolInfo || isOwner">
      ...
      <div v-if="profileUser.highSchool" class="flex justify-between items-center mb-4">
         <div class="flex items-center text-gray-900">
            <div class="mr-3"><School class="text-gray-400 text-4xl" /></div>
            <span>Uczęszczał do: <strong>{{ profileUser.highSchool }}</strong></span>
         </div>
         <div v-if="isOwner" class="flex items-center space-x-2 text-gray-500">
            <Earth class="text-lg"/>
            <ItemMenu editText="Edytuj szkołę średnią" removeText="Usuń szkołę średnią" @edit="editHighSchool" @remove="remove('highSchool')"/>
         </div>
      </div>

      <div class="mt-2">
        <EditForm v-if="activeSection === 'highSchool'" label="Szkoła" v-model="form.highSchool" @cancel="close" @save="save('highSchool')" />
