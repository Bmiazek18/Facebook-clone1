    ...
    <div>
      ...
      <div v-if="activeSection === 'rel'" class="bg-gray-50 p-4 rounded-lg mb-4">
         <div class="relative bg-white border border-gray-300 rounded-lg px-3 py-2 mb-4">
             <select v-model="form.status" class="w-full bg-transparent outline-none"><option>{{ $t('profile.info.status.single') }}</option><option>{{ $t('profile.info.status.inRelationship') }}</option></select>
         </div>
         <div class="flex justify-end space-x-2"><button @click="close" class="px-3 py-1 bg-gray-200 rounded">{{ $t('common.cancel') }}</button><button @click="save" class="px-3 py-1 bg-blue-600 text-white rounded">{{ $t('common.save') }}</button></div>
      </div>
      ...
      <div v-else-if="profileUser.relationshipStatus" class="flex justify-between items-center mb-4">
         ...
         <div v-if="isOwner" class="flex items-center space-x-2 text-gray-500"><Earth class="text-lg"/><ItemMenu :editText="$t('common.edit')" :removeText="$t('common.delete')" @edit="openRel" @remove="close"/></div>
      </div>

      <button v-if="isOwner && !profileUser.relationshipStatus && activeSection !== 'rel'" @click="openRel" class="flex items-center text-blue-600 hover:underline font-medium"><PlusCircleOutline class="mr-3 text-2xl" /> {{ $t('profile.info.addRelationshipStatus') }}</button>
    </div>
    ...
    <div>
      ...
      <div v-for="(member, idx) in profileUser.familyMembers" :key="idx">
         ...
         <div v-if="activeSection === 'fam_edit' && editingIndex === idx" class="bg-gray-50 p-4 rounded-lg mb-4">
             <div class="mb-3 border-2 border-blue-500 rounded-lg px-3 py-2 bg-white"><input v-model="form.familyName" class="w-full outline-none" :placeholder="$t('common.name')"/></div>
             <div class="bg-gray-200 rounded-lg px-3 py-2 mb-4"><select v-model="form.relation" class="w-full bg-transparent outline-none"><option>{{ $t('profile.info.brother') }}</option><option>{{ $t('profile.info.sister') }}</option></select></div>
             <div class="flex justify-end space-x-2"><button @click="close" class="px-3 py-1 bg-gray-200 rounded">{{ $t('common.cancel') }}</button><button @click="save" class="px-3 py-1 bg-blue-600 text-white rounded">{{ $t('common.save') }}</button></div>
         </div>
         ...
         <div v-else class="flex justify-between items-center mb-4">
            ...
            <div v-if="isOwner" class="flex items-center space-x-2 text-gray-500"><Earth class="text-lg"/><ItemMenu :editText="$t('common.edit')" :removeText="$t('common.delete')" @edit="editFamily(idx, member)" @remove="close"/></div>
         </div>
      </div>
      ...
      <div v-if="activeSection === 'fam_add'" class="bg-gray-50 p-4 rounded-lg mt-2 relative">
         <div class="mb-3 border-2 border-blue-500 rounded-lg px-3 py-2 bg-white relative">
            <input v-model="form.familyName" class="w-full outline-none" :placeholder="$t('common.name')" @input="showDropdown=true" @blur="setTimeout(()=>showDropdown=false, 200)"/>
      ...
      <button v-else-if="isOwner" @click="addFamily" class="flex items-center text-blue-600 hover:underline font-medium"><PlusCircleOutline class="mr-3 text-2xl" /> {{ $t('profile.info.addFamilyMember') }}</button>
    </div>
  </div>
</template>
