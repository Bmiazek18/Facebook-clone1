...
const hasMedia = computed(() => selectedImageUrls.value.length > 0  || newMessage.value.trim().length > 0);
  ...
  >
    <transition name="reply">
      <div v-if="props.reply" class="reply-preview">
        ...
        <button @click="$emit('clearReply')" class="reply-close-btn text-theme-text hover:text-theme-text-secondary transition-colors">
        ...
        <div class="flex items-center gap-1 shrink-0 pb-1 mr-1">
          ...
          >
            <div v-show="!hasMedia" class="flex items-center gap-1">
              <div v-tooltip.top="'Zdjęcia'" class="p-1 rounded-full hover:bg-black/5 cursor-pointer">
                <ImageOutlineIcon
                  :size="24"
                  :style="{ color: props.themes?.iconColor }"
                  @click="selectImage"
              ...
              <div v-tooltip.top="'Naklejki'" class="p-1 rounded-full hover:bg-black/5 cursor-pointer">
              ...
              >
                 ...
                 <div v-tooltip.top="'GIF'"  :style="{ backgroundColor: props.themes?.iconColor || '#3b82f6' }" class="text-white text-[10px] font-black p-1 rounded flex ml-1 items-center justify-center h-[20px] w-[24px] leading-none  hover:bg-black/5 cursor-pointer">
              GIF
            </div>

                <template #popper>
                  <div class="gif-popper-content">
                    <GifBox :theme-color="props.themes?.iconColor" @handleGifSelection="handleGifSelect" />
                  </div>
                </template>
              </VDropdown>
            </div>
...
<div v-show="hasMedia" class="flex items-center">
  ...
  >
    ...
    >
      ...
      <template #popper>
        <div class="flex flex-col py-2 min-w-[280px] rounded-xl shadow-2xl border border-gray-100 overflow-hidden bg-white">

          <button @click="showVoiceRecorder = true" class="flex items-center gap-4 w-full px-4 py-2 hover:bg-gray-100 transition-colors group text-left">
            <microphone-icon :size="24" :fillColor="props.themes?.iconColor" />
            <span class="text-gray-800 font-medium text-[15px]">Prześlij nagranie głosowe</span>
          </button>

          <button @click="selectImage" class="flex items-center gap-4 w-full px-4 py-2 hover:bg-gray-100 transition-colors group text-left">
            <image-multiple-icon :size="24" :fillColor="props.themes?.iconColor" />
            <span class="text-gray-800 font-medium text-[15px]">Załącz plik o wielkości do 100 MB</span>
          </button>

          <button class="flex items-center gap-4 w-full px-4 py-2 hover:bg-gray-100 transition-colors group text-left">
          ...
          <button @click="openGifFromPlus" class="flex items-center gap-4 w-full px-4 py-2 hover:bg-gray-100 transition-colors group text-left">
            <div :style="{ backgroundColor: props.themes?.iconColor || '#3b82f6' }" class="text-white text-[10px] font-black px-1 rounded flex items-center justify-center h-[20px] w-[24px] leading-none">
              GIF
            </div>
    ...
    <template #popper>
      <div class="gif-popper-content">
        ...
        >
          <div v-if="selectedImageUrls.length > 0  " class="flex items-center gap-2 pt-2 px-3 pb-0 overflow-x-auto">
            ...
            <div v-for="(img, index) in selectedImageUrls" :key="index" class="relative w-12 h-12 shrink-0 mt-2 mb-1">
              <img :src="img" class="w-full h-full object-cover rounded-[10px] border border-black/5" />
              <button
            ...
            <div v-if="selectedGifUrl" class="relative w-12 h-12 shrink-0 mt-2 mb-1">
              <img :src="selectedGifUrl" class="w-full h-full object-cover rounded-[10px] border border-black/5" />
              <button
            ...
            >
              ...
              <template #popper>
                <div class="emoji-popper-content">
                  <LazyEmojiPicker @select="(e) => { newMessage += e.native;  }" />
                </div>
              </template>
            </VDropdown>
          </div>
        ...
        <div class="flex items-center shrink-0 ">
          <LikeButton v-if="!isVoiceRecording && !newMessage.length && !hasMedia" :emoji="localSelectedEmoji" @send-like="handleSendLike" />
          <div v-if="!isVoiceRecording && (newMessage.length || hasMedia)" v-tooltip.top="'Wyślij'" class="w-[40px] h-[40px] flex align-center items-center justify-center" >
            <SendIcon :size="22" class="cursor-pointer  " :fillColor="props.themes?.iconColor || '#3b82f6'" @click="sendMessage(newMessage, 'default')" />
          </div>
        </div>
      </template>
    </div>
...
.icon-state-0 { width: 24px; height: 24px; font-size: 24px; transition: all 0.3s ease-out; }
.icon-state-1 { width: 30px; height: 30px; font-size: 30px; transition: all 0.3s ease-out; }
.icon-state-2 { width: 36px; height: 36px; font-size: 36px; transition: all 0.3s ease-out; }
.icon-state-3 { width: 48px; height: 48px; font-size: 48px; transition: all 0.3s ease-out; }
