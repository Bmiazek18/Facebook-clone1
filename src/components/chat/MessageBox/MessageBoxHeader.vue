...
const otworzOknoRozmowy = async (typPolaczenia: 'audio' | 'video') => {
  // 1. Rejestracja połączenia w Redisie (TTL = 30s)
  try {
    await $fetch('http://localhost:8080/api/chat/calls/start', {
      method: 'POST',
      query: {
  ...
  if (conversationsStore.isMqttConnected) {
    const payload = {
      ...
      senderId: conversationsStore.currentUserUuid,
      callerId: conversationsStore.currentUserUuid,
      callType: typPolaczenia,
      participantIds: [conversationsStore.currentUserUuid, props.boxId.toString()],
    }
    conversationsStore.publishMqtt('chat/messages/inbound', payload)
    console.log('Published call_initiated MQTT event:', payload)
  }

  // Generujemy URL z routera
  ...
  >
    <div class="flex items-center gap-2 min-w-0">
      ...
      <div
        ...
        class="relative shrink-0 w-8 h-8 rounded-full overflow-hidden bg-black/5 flex items-center justify-center border border-black/10"
      >
