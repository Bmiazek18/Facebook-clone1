<template>
  <div class="w-full h-full max-w-md mx-auto overflow-hidden h-[800px] flex flex-col  ">
    <div class="p-3 border-b border-gray-100 flex items-center gap-3">
      <button
        @click="emit('close')"
        class="text-black hover:bg-gray-100 p-2 rounded-full transition duration-200"
      >
        <svg
    ...
    <div class="px-4 py-2">
      <div
        class="flex items-center bg-gray-100 rounded-full px-3 py-1.5 transition-colors focus-within:bg-gray-200/70"
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
        ...
        <div
          v-if="searchQuery"
          class="bg-gray-300/60 text-gray-600 text-[11px] font-semibold px-2 py-0.5 rounded-md whitespace-nowrap ml-2"
        >
...
const sourceMessages = computed(() => {
  if (props.boxId !== undefined && props.boxId !== null) {
    const msgs = convStore.getMessagesByChatId(props.boxId)
