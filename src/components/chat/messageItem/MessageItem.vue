...
const getTooltipText = (userId: string) => {
  ...
  const convStore = useConversationsStore()
  const user = convStore.usersCache[cleanId]
  if (!user) convStore.getOrFetchUser(cleanId)
  const name = user ? user.name : 'Użytkownik'
...
<template>
  ...
  >
      ...
      >
        ...
        >
          <span
            ...
            class="text-[10px] absolute -top-5 right-0"
            :style="{ color: theme.timestampColor }"
          >
            Przypieta</span
          >
          <PinIcon
          ...
          <ChatMessageImage
            ...
            :message="message"
            :is-me="isMe"
            @open-lightbox="chatActions?.openLightbox($event)"
          />
          ...
          >
            ...
            <span v-if="totalReactions > 1" class="text-[9px] font-bold text-gray-500 ml-0.5">{{
              totalReactions
            }}</span>
          </div>
        </div>
    ...
    <div
      :class="{
        ' h-4 mt-1 ':
          displayReadBy.visible.length > 0 || displayReadBy.hiddenCount > 0 || displayTimeAgo,
      }"
      class="h-0 transition-all duration-300 flex justify-end w-full relative"
    >
      <div
        v-if="!(displayReadBy.visible.length > 0 || displayReadBy.hiddenCount > 0)"
        :style="{ color: props.theme.timestampColor }"
        class="text-[12px]"
      >
        {{ displayTimeAgo }}
      </div>
      ...
      >
        ...
        <img
          ...
          v-tooltip="getTooltipText(userId)"
  ...
  <BaseModal v-if="showReactionsPanel" @close="showReactionsPanel = false" title="Reakcje">
    <div class="p-4 max-h-[400px] overflow-y-auto min-w-[280px]">
      <div v-if="!message.reactions || Object.keys(message.reactions).length === 0" class="text-center text-gray-500 py-4">
        Brak reakcji na tę wiadomość
      </div>
          ...
          <div class="space-y-2 pl-2">
            <template v-if="Array.isArray(userIds)">
              <div v-for="userId in userIds" :key="userId" class="flex items-center space-x-3">
                <img
                  :src="getUserAvatar(userId)"
                  class="w-8 h-8 rounded-full object-cover shadow-sm bg-gray-100 border border-gray-200"
                  alt="avatar"
                />
            ...
            <template v-else>
              <div class="flex items-center space-x-3">
                <img
                  :src="getUserAvatar(String(userIds))"
                  class="w-8 h-8 rounded-full object-cover shadow-sm bg-gray-100 border border-gray-200"
                  alt="avatar"
                />
                <span class="text-sm font-medium text-gray-700">
                  {{ getUserName(String(userIds)) }}
                </span>
              </div>
            </template>
          </div>
...
@keyframes pulsowanie {
  ...
  50% {
    transform: scale(
      1.1
    ); /* Powiększenie odrobinę zmniejszone, by animacja w dużym dymku nie była zbyt agresywna */
  }
  100% {
