...
<template>
  <div class="overflow-visible">
    ...
    <div
      v-else-if="isGroupedImage && (message.mediaUrls?.length ?? 0) >= 2 && (message.mediaUrls?.length ?? 0) <= 3"
    ...
    <div v-else-if="isGroupedImage && (message.mediaUrls?.length ?? 0) > 3" class="relative w-[140px] h-[100px] cursor-pointer">
      <img
        @click="emit('open-lightbox', message.mediaUrls?.[0] ?? '')"
        :src="message.mediaUrls?.[0]"
