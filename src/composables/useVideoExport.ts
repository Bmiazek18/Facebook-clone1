...
const calculateTextState = (text: TextOverlay, time: number): AnimationState => {
  ...
  if (text.entryAnimation && text.entryAnimation !== 'none' && timeSinceStart < entryDuration) {
    ...
    switch (text.entryAnimation) {
      case 'fade-in': opacity *= progress; break;
      case 'zoom-in': scale = 0.2 + (0.8 * easeVal); opacity *= progress; break;
      case 'pop-in': scale = Math.max(0, easeOutBack(progress)); break;
      case 'slide-in-left': translateX = -100 * (1 - easeVal); break;
      case 'slide-in-right': translateX = 100 * (1 - easeVal); break;
      case 'slide-in-top': translateY = -100 * (1 - easeVal); break;
      case 'slide-in-bottom': translateY = 100 * (1 - easeVal); break;
    }
  }
  ...
  if (text.exitAnimation && text.exitAnimation !== 'none' && timeUntilEnd < exitDuration) {
    ...
    switch (text.exitAnimation) {
      case 'fade-out': opacity *= progress; break;
      case 'zoom-out': scale *= progress; opacity *= progress; break;
      case 'slide-out-left': translateX -= 100 * (1 - easeVal); break;
      case 'slide-out-right': translateX += 100 * (1 - easeVal); break;
      case 'slide-out-top': translateY -= 100 * (1 - easeVal); break;
      case 'slide-out-bottom': translateY += 100 * (1 - easeVal); break;
      case 'pop-out': scale *= progress; break;
    }
  }
      ...
      for (let i = 0; i < totalFrames; i++) {
        ...
        if (activeClip) {
          if (currentClipUrl !== activeClip.url) {
            ...
            await new Promise(r => { videoEl.onloadeddata = () => r(true); });
          }
          ...
          const scale = Math.max(CANVAS_WIDTH / videoEl.videoWidth, CANVAS_HEIGHT / videoEl.videoHeight);
        ...
        const activeTexts = textOverlays.value.filter(t => time >= t.startTime && time <= t.endTime);
        for (const text of activeTexts) {
          ...
          ctx.translate((state.translateX / 100) * CANVAS_WIDTH, (state.translateY / 100) * CANVAS_HEIGHT);
        ...
        const activeImages = imageOverlays.value.filter(img => time >= img.startTime && time <= img.endTime);
        ...
        const activePipVideos = pipVideoOverlays.value.filter(vid => time >= vid.startTime && time <= vid.endTime);
        for (const pipVideo of activePipVideos) {
          ...
          if (tempCtx) {
            ...
            tempCtx.quadraticCurveTo(pipVideo.width, pipVideo.height, pipVideo.width - radius, pipVideo.height);
          ...
          ctx.drawImage(tempCanvas, -pipVideo.width / 2, -pipVideo.height / 2, pipVideo.width, pipVideo.height);
          ctx.restore();
        }
