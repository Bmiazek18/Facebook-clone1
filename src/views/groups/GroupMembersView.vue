...
const friendsInGroup = ref([
  ...
  {
    id: 202,
    name: 'Paweł Sitkowski',
    subtext: 'AWF Biała Podlaska',
    avatar: 'https://via.placeholder.com/150',
  },
])
...
const newMembers = ref([
  ...
  {
    id: 33,
    name: 'Kacper Wawryszewicz',
    joined: 'około 5 mies. temu',
    extra: 'Terespol',
    action: 'add',
  },
  {
...
<template>
  <div
    class="min-h-screen bg-theme-bg-secondary text-theme-text p-4   max-w-[680px] mx-auto"
  >
    <section class="mb-6">
      <div class="flex items-center mb-4">
        <h2 class="text-[20px] font-bold">Członkowie</h2>
        <span class="text-[20px] font-bold text-theme-text-secondary ml-2"
      ...
      <p class="text-[15px] text-theme-text-secondary mb-3">
        ...
        <span class="text-theme-primary hover:underline cursor-pointer font-semibold"
    ...
    <section class="mb-6">
      <div
        ...
        :key="'top-' + member.id"
        class="flex items-center justify-between py-2 border-t border-theme-border pt-4"
      >
        <div class="flex items-center space-x-3">
          <img :src="member.avatar" class="w-12 h-12 rounded-full object-cover" />
          <div>
            <div class="font-bold text-[15px] hover:underline cursor-pointer">
              {{ member.name }}
            </div>
    ...
    <section class="mb-8">
      ...
      <div v-for="admin in admins" :key="admin.id" class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
          ...
          <div>
            ...
            <div class="flex flex-wrap items-center gap-1 mt-0.5">
              <span
                class="bg-theme-primary-opacity-20 text-theme-primary text-[12px] px-1.5 py-0.5 rounded font-semibold"
                >{{ admin.role }}</span
              >
              <span
                class="bg-theme-bg-subtle text-theme-text text-[12px] px-1.5 py-0.5 rounded flex items-center"
              >
        ...
        <button
          class="bg-theme-bg-subtle hover:bg-theme-hover-strong text-theme-text px-3 py-2 rounded-md font-semibold text-[14px] flex items-center whitespace-nowrap"
        >
          <span class="mr-2">👤<span class="ml-[-4px] text-[10px]">×</span></span> Anuluj
          zaproszenie
        </button>
      </div>
    </section>
    ...
    <section class="mb-8">
      <h3 class="text-[17px] font-bold mb-1 text-theme-text">
        · 1 uczestnik z punktami za wkład w grupę
      </h3>
      <p class="text-[13px] text-theme-text-secondary mb-4">
        To są członkowie grupy, którzy zdobyli punkty za wkład w nią, z sortowaniem malejącym.
      </p>
      <div
        v-for="member in activeMembers"
    ...
    <section class="mb-8">
      ...
      <div class="space-y-4">
        <div
          v-for="friend in friendsInGroup"
          :key="friend.id"
          class="flex items-center justify-between"
        >
          <div class="flex items-center space-x-3">
            ...
            <div>
              ...
              <div v-if="friend.subtext" class="text-[13px] text-theme-text-secondary">
                {{ friend.subtext }}
              </div>
            </div>
          </div>
          <button
            class="bg-theme-bg-subtle hover:bg-theme-hover-strong text-theme-text px-3 py-2 rounded-md font-semibold text-[14px] flex items-center transition"
          >
      ...
      <button
        class="w-full mt-6 bg-theme-bg-subtle-opacity-60 hover:bg-theme-hover-strong transition py-2 rounded-md font-semibold text-[15px] text-theme-text"
      >
        Wyświetl wszystkich
      </button>
    </section>
    ...
    <section class="mb-8">
      ...
      <div class="space-y-6">
        ...
        >
          ...
          <button
            class="bg-theme-bg-subtle hover:bg-theme-hover-strong text-theme-text px-3 py-1.5 rounded-md font-semibold text-[14px] flex items-center shrink-0"
          >
            <span class="mr-1.5 text-lg">👤<span class="ml-[-4px] text-[10px]">+</span></span> Dodaj
            znajomego
          </button>
        </div>
      </div>
    ...
    <section class="mb-8">
      ...
      <p class="text-[13px] text-theme-text-secondary mb-4 leading-snug">
        Ta lista obejmuje osoby, które dołączyły do grupy, a także osoby wyświetlające podgląd tej
        grupy. Każda zaproszona i zatwierdzona osoba może wyświetlać podgląd zawartości grupy.
      </p>
      ...
      <div class="space-y-4">
        ...
        >
          <div class="flex items-center space-x-3">
            <div
              v-if="!member.extra"
              class="w-12 h-12 rounded-full bg-theme-bg-subtle flex items-center justify-center overflow-hidden"
            >
              <svg
                class="w-10 h-10 text-theme-text-secondary mt-2"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
            ...
            <div>
              ...
              <div v-if="member.extra" class="text-[13px] text-theme-text-secondary">
                {{ member.extra }}
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center">
            <button
              v-if="member.action === 'message'"
              class="bg-theme-bg-subtle hover:bg-theme-hover-strong text-theme-text px-3 py-1.5 rounded-md font-semibold text-[14px] flex items-center"
            >
              <span class="mr-2">💬</span> Wiadomość
            </button>
            <button
              v-else-if="member.action === 'add'"
              class="bg-theme-bg-subtle hover:bg-theme-hover-strong text-theme-text px-3 py-1.5 rounded-md font-semibold text-[14px] flex items-center"
            >
              <span class="mr-1.5 text-lg leading-none"
                >👤<span class="ml-[-4px] text-[10px]">+</span></span
              >
              Dodaj znajomego
            </button>
          </div>
