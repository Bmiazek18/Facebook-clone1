...
const privacyOptions = [
  { id: 'public', title: 'Publiczna', description: 'Każdy może sprawdzić listę członków grupy i zobaczyć ich posty.', icon: Earth },
  { id: 'private', title: 'Prywatna', description: 'Tylko członkowie grupy mogą sprawdzić listę członków grupy i zobaczyć ich posty.', icon: Lock },
];

const visibilityOptions = [
  { id: 'visible', title: 'Widoczna', description: 'Każdy może znaleźć tę grupę.', icon: Eye },
  { id: 'hidden', title: 'Ukryta', description: 'Tylko członkowie mogą znaleźć tę grupę.', icon: EyeOff },
];
...
const titleColorClass = computed(() => groupName.value.trim() ? 'text-theme-text' : 'text-theme-text-secondary');
  ...
  <div class="flex h-screen bg-theme-bg font-sans text-theme-text overflow-hidden">
    ...
    <aside class="w-[360px] bg-theme-bg-secondary flex flex-col shadow-theme-shadow z-20 shrink-0 h-full border-r border-theme-border" @click.stop>
      ...
      <div class="flex-1 overflow-y-auto px-4 py-5 custom-scrollbar relative">
        ...
        <div class="mb-4 relative">
            <CustomDropdown v-model="privacy" :options="privacyOptions" label="Wybierz ustawienie prywatności" />
        </div>
        ...
        <div class="mb-2">
           <div class="w-full min-h-[56px] px-2 py-1 border border-theme-border rounded-lg focus-within:border-theme-primary focus-within:ring-1 focus-within:ring-theme-primary hover:border-theme-border-hover flex flex-wrap items-center gap-1 bg-theme-bg-secondary">
              ...
              <input type="text" v-model="inviteInput" placeholder="Zaproś znajomych (opcjonalne)" class="flex-1 h-[40px] px-2 outline-none text-[15px] min-w-[120px] bg-transparent">
           </div>
        </div>
      ...
      <div class="p-4 border-t border-theme-border shadow-theme-shadow-sm bg-theme-bg-secondary z-10">
        <button class="w-full h-9 rounded-md font-semibold text-[15px] flex items-center justify-center transition-colors" :class="isFormValid ? 'bg-theme-primary text-white hover:bg-theme-primary-hover' : 'bg-theme-bg-tertiary text-theme-text-placeholder cursor-not-allowed'" :disabled="!isFormValid">
          Utwórz
        </button>
      </div>
    </aside>
    ...
    <main class="flex-1 flex justify-center items-start pt-8 pb-8 px-8 overflow-y-auto bg-theme-bg">
      ...
      >
        ...
        <div class="flex justify-between items-center px-4 h-[60px] border-b border-theme-border bg-theme-bg-secondary shrink-0">
            ...
            <div class="flex items-center gap-2">
                <button
                    ...
                    :class="!isPreviewMobile ? 'text-theme-primary bg-theme-blue-light' : 'text-theme-text-secondary hover:bg-theme-bg-hover'"
        ...
        <div class="flex-1 flex flex-col overflow-y-auto custom-scrollbar bg-theme-bg-secondary cursor-default">

            <div :class="isPreviewMobile ? 'p-0' : 'p-4'">
                <div :class="['bg-theme-bg overflow-hidden relative', isPreviewMobile ? '' : 'rounded-lg aspect-[2.5/1]', isPreviewMobile ? 'aspect-[2/1]' : '']">
                    <img :src="coverIllustration" class="w-full h-full object-cover filter grayscale opacity-90 drag-none" style="-webkit-user-drag: none;">
                </div>
            </div>
            ...
            <div :class="isPreviewMobile ? 'px-4 pt-3 pb-0' : 'px-8 pt-4 pb-0'">
                ...
                <div class="flex mt-4 border-t border-theme-border overflow-x-auto no-scrollbar justify-between md:justify-start">
                    <div :class="isPreviewMobile ? 'px-2 py-3 text-[14px]' : 'px-4 py-4 text-[15px]'" class="text-theme-text-secondary mx-1 my-1 font-semibold whitespace-nowrap">Informacje</div>
                    <div :class="isPreviewMobile ? 'px-2 py-3 text-[14px]' : 'px-4 py-4 text-[15px]'" class="text-theme-text-secondary font-semibold mx-1 my-1 whitespace-nowrap">Posty</div>
                    <div :class="isPreviewMobile ? 'px-2 py-3 text-[14px]' : 'px-4 py-4 text-[15px]'" class="text-theme-text-secondary font-semibold mx-1 my-1 whitespace-nowrap">Członkowie</div>
                    <div :class="isPreviewMobile ? 'px-2 py-3 text-[14px]' : 'px-4 py-4 text-[15px]'" class="text-theme-text-secondary font-semibold mx-1 my-1 whitespace-nowrap">Wydarzenia</div>
                </div>
            </div>

            <div :class="['bg-theme-bg p-4 flex gap-4 overflow-y-auto custom-scrollbar flex-1', isPreviewMobile ? 'flex-col' : 'flex-row items-start']">

                <div class="flex-1 shrink-0">
                    <div class="bg-theme-bg-secondary rounded-lg shadow-sm border border-theme-border px-4 py-3 grayscale opacity-50">
                        <div class="flex items-center gap-2 mb-3 grayscale">
                            <AccountCircle :size="40" class="text-theme-bg-tertiary" />
                            <div class="bg-theme-bg-tertiary rounded-full flex-1 px-4 py-2 text-theme-text-placeholder text-[15px] text-left truncate">
                                Co słychać?
                            </div>
                        </div>

                        <div class="border-t border-theme-border pt-2 flex justify-between">
                            <div class="flex items-center justify-center flex-1 py-2 rounded-md px-1 opacity-60 grayscale">
                                <ImageMultiple class="text-theme-text-secondary mr-2 shrink-0" :size="24" />
                                <span class="text-theme-text-secondary font-semibold text-[14px] truncate">Zdjęcie/film</span>
                            </div>

                            <div class="flex items-center justify-center flex-1 py-2 rounded-md px-1 opacity-60 grayscale">
                                <AccountTag class="text-theme-text-secondary mr-2 shrink-0" :size="24" />
                                <span class="text-theme-text-secondary font-semibold text-[14px] truncate">Oznacz osoby</span>
                            </div>

                            <div class="flex items-center justify-center flex-1 py-2 rounded-md px-1 opacity-60 grayscale">
                                <EmoticonOutline class="text-theme-text-secondary mr-2 shrink-0" :size="24" />
                                <span class="text-theme-text-secondary font-semibold text-[14px] truncate">Nastrój/akt...</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="fixed bottom-6 right-6 z-50">
      <button class="bg-theme-bg-secondary p-3 rounded-full shadow-lg border border-theme-border hover:bg-theme-bg-hover transition">
