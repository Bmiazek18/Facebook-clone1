...
<template>
  <div class="flex h-screen w-full bg-theme-bg   text-theme-text overflow-hidden">
    ...
    <div class="flex h-full w-full">
      <aside
        class="w-[360px] flex-shrink-0 bg-theme-bg-secondary flex flex-col border-r border-theme-border h-full"
      >
        ...
        <nav class="px-2 space-y-1">
          <div class="pb-2">
            <div
              class="flex items-center gap-3 px-3 py-2 rounded-lg cursor-default bg-gray-100 dark:bg-blue-900/20 transition-colors"
            >
              <div
                class="w-10 h-10 rounded-full flex items-center justify-center bg-[#1877F2] text-white shadow-sm"
              >
          ...
          <div class="pt-1 space-y-1">
            ...
            >
              <div
                class="w-10 h-10 rounded-full flex items-center justify-center bg-theme-bg-tertiary group-hover:bg-gray-300 dark:group-hover:bg-zinc-600 transition-colors text-theme-text"
              >
      ...
      <main class="flex-1 flex flex-col relative bg-theme-bg">
        <div class="flex-1 flex flex-col items-center justify-center p-8">
          ...
          <div class="flex flex-wrap justify-center gap-4">
            ...
            >
              <div class="mb-5 mt-2">
                <div
                  :class="`w-16 h-16 rounded-full ${item.iconBg} flex items-center justify-center text-gray-900`"
                >
                  <component :is="item.icon" :size="32" />
                </div>
              </div>

              <h3 class="font-bold text-[17px] mb-2 text-theme-text leading-tight">
                {{ item.title }}
              </h3>
              <p class="text-[13px] text-theme-text-secondary leading-snug px-1">
                {{ item.description }}
              </p>
            </div>
          </div>
        ...
        <button
          class="absolute bottom-8 right-8 w-12 h-12 bg-theme-bg-tertiary rounded-full flex items-center justify-center hover:bg-theme-hover shadow-lg border border-theme-border text-theme-text transition-transform hover:scale-105"
        >
          <PencilIcon :size="24" />
        </button>
      </main>
