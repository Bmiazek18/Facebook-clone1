...
import MediaLightbox from '@/components/messenger/MessageBox/MediaLightbox.vue';
...
const conditions: DropdownOption[] = [
  ...
  { id: 'like_new', title: 'Używany - jak nowy', description: 'Przedmiot używany, w doskonałym stanie.' },
  { id: 'good', title: 'Używany - dobry', description: 'Przedmiot używany, w dobrym stanie.' },
  { id: 'acceptable', title: 'Używany - akceptowalny', description: 'Przedmiot używany, w akceptowalnym stanie.' },
];
...
const closeSellerModal = () => { isSellerModal.value = false; };
...
<template>
  <div class="flex h-screen w-full bg-theme-bg font-sans text-theme-text overflow-hidden">
    ...
    <aside class="w-[360px] flex-shrink-0 bg-theme-bg-secondary shadow-sm flex flex-col border-r border-theme-border z-20 h-full">
<AppCloseHeader class="shadow-sm pb-2"/>
      <div class="px-4 pt-4 pb-2 flex-shrink-0">
        <div class="flex justify-between items-start mb-1">
           ...
           <button class="text-[15px] text-theme-primary font-semibold hover:underline">Zapisz wersję roboczą</button>
        </div>
        <h1 class="text-2xl font-bold leading-tight mb-4">Przedmiot na sprzedaż</h1>

        <div class="flex items-center gap-3 mb-4">
          <img :src="auth.currentUser?.avatar " alt="Avatar" class="w-10 h-10 rounded-full border border-gray-100" />
          <div>
...
>
  ...
  >
    ...
    <div v-if="uploadedImages.length > 0" class="flex gap-3 overflow-x-auto pb-2">
      <div v-for="(img, index) in uploadedImages" :key="index" class="relative w-24 h-24 flex-shrink-0 bg-gray-100 rounded-lg border border-gray-200 overflow-hidden group/item">
        <img :src="img" class="w-full h-full object-cover" />
        <button @click.stop="removeImage(index)" class="absolute top-1 right-1 bg-black/50 text-white rounded-full p-1 opacity-0 group-hover/item:opacity-100 transition-opacity">
          <CloseIcon :size="14" />
        </button>
      </div>
    </div>
    ...
    >
      ...
      />
        ...
        <div class="bg-theme-bg rounded-lg p-3 flex items-center justify-between">
           <div class="flex items-center gap-3">
              ...
              <div>
                 ...
                 <a href="#" class="text-[13px] text-theme-primary hover:underline">Dowiedz się więcej</a>
              </div>
           </div>
           <button class="bg-theme-hover-strong hover:bg-[#cdd0d5] text-theme-text text-[13px] font-semibold px-3 py-1.5 rounded-md transition">
             Wypróbuj
           </button>
        </div>
        ...
        <div class="space-y-3">
          <div class="mb-1">
            ...
            <p class="text-[13px] text-theme-text-secondary">Opis powinien być jak najbardziej szczegółowy.</p>
          </div>
          ...
          <div class="border-t border-theme-border pt-4 mt-2">
             <div @click="showMoreInfo = !showMoreInfo" class="flex justify-between items-center cursor-pointer">
                <div>
                   <h3 class="font-bold text-[17px]">Więcej informacji</h3>
                   <p class="text-[13px] text-theme-text-secondary mt-0.5 max-w-[280px]">Wzbudź większe zainteresowanie, podając więcej szczegółów informacji.</p>
                </div>
                <div class="p-2 bg-theme-bg-subtle rounded-full transition-transform" :class="{ 'rotate-180': showMoreInfo }">
             ...
             <div class="flex items-center justify-between pb-4">
                <div class="flex gap-3 items-center">
                   ...
                   <div>
                      ...
                      <p class="text-[12px] text-theme-text-secondary mt-0.5">To ogłoszenie jest nadal publiczne.</p>
                   </div>
                </div>
                <div class="w-12 h-7 bg-theme-border rounded-full relative cursor-pointer">
                   <div class="w-5 h-5 bg-white rounded-full shadow absolute top-1 left-1"></div>
      ...
      <div class="px-4 pb-4 pt-2 border-t border-theme-border bg-theme-bg-secondary shadow-[0_-2px_5px_rgba(0,0,0,0.05)] z-30">
        ...
        <button
          ...
          :class="form.title ? 'bg-theme-primary text-white hover:bg-theme-primary-hover' : 'bg-theme-bg-subtle text-theme-text-secondary cursor-not-allowed'"
          :disabled="!form.title"
        >
          Dalej
        </button>
    ...
    <main class="flex-1 flex flex-col items-center justify-center p-8 relative overflow-hidden bg-theme-bg">
      ...
      <div class="w-full h-[90vh] max-w-[980px] z-10 flex flex-col items-center">
        ...
        <div class="bg-theme-bg-secondary rounded-lg shadow-sm border border-theme-border w-full h-full flex overflow-hidden">
          ...
          >
             ...
             <div v-else class="max-w-[260px] select-none">
                <h3 class="text-[17px] font-bold text-theme-text-secondary mb-2">Podgląd Twojego ogłoszenia</h3>
          ...
          <div class="w-[40%] flex flex-col h-full bg-theme-bg-secondary">
            ...
            <div class="p-5 flex-1 overflow-y-auto space-y-4 scrollbar-visible">
              ...
              <div class="transition-all duration-300 rounded-lg py-2" :class="getHighlightClass('description')">
                 <p class="text-[15px] text-theme-text leading-relaxed break-words whitespace-pre-line">
              ...
              <div class="transition-all duration-300 py-1 rounded-lg" :class="focusedField ? 'opacity-30 blur-[1px]' : 'opacity-100'">
                <div class="flex justify-between items-center mb-4">
                  ...
                  <a href="#" class="text-[15px] text-theme-primary font-semibold hover:underline">Szczegóły</a>
                </div>
                <div @click="openSellerModal" class="flex items-center gap-3 cursor-pointer">
                  ...
                  <div>
                    ...
                    <p class="text-[13px] text-theme-text-secondary">Dołączył(a) do Facebooka w 2015</p>
                  </div>
                </div>
              </div>
            ...
            <div class="p-4 border-t border-theme-border bg-theme-bg-secondary z-20">
              <button disabled class="w-full bg-theme-bg-subtle text-theme-text-secondary font-bold py-2 rounded-md mb-3 cursor-not-allowed select-none transition-colors">
                Wyślij wiadomość
              </button>
              <p class="text-[11px] text-theme-text-secondary text-center leading-[1.2] px-1">
                <a href="#" class="text-theme-primary hover:underline">Dowiedz się więcej</a> about purchasing from consumers.
              </p>
            </div>
