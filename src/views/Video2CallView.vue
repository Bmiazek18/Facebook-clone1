...
import AgoraRTC, { IAgoraRTCClient, IMicrophoneAudioTrack, ILocalVideoTrack, IRemoteUser } from 'agora-rtc-sdk-ng'
...
const AGORA_APP_ID = '24661afad47b453aa34ac2f64ba8c614'
...
const remoteVideoUser = ref<IRemoteUser | null>(null)
...
const initializeAgoraAndAI = async () => {
  ...
  try {
    ...
    if (videoRef.value) {
      ...
      await new Promise((resolve) => { videoRef.value!.onloadedmetadata = () => resolve(true) })
    ...
    await agoraClient.join(AGORA_APP_ID, String('318509'), AGORA_TOKEN, null)
...
<template>
  <div v-if="currentStep === 'call'" class="relative w-full h-screen bg-black text-white flex items-center justify-center overflow-hidden font-sans">
    ...
    <div class="relative h-full w-full max-w-[500px] bg-black flex items-center justify-center overflow-hidden">
      <img :src="avatarUrl" alt="Blur Background" class="absolute inset-0 w-full h-full object-cover blur-3xl scale-110 opacity-40" />
      <div class="absolute inset-0 bg-black/50"></div>

      <div v-show="remoteVideoUser" ref="remoteVideoRef" class="absolute inset-0 w-full h-full object-cover z-10"></div>
      ...
      <div v-show="remoteVideoUser && remoteUserSpeaking" class="absolute inset-0 border-4 border-green-500 pointer-events-none z-20 transition-all"></div>
    </div>

    <div v-if="connectionType === 'video'" class="absolute bottom-6 right-6 z-20 flex items-center gap-3">
      <button
      ...
      <div class="w-48 aspect-[16/10] bg-zinc-900 rounded-xl overflow-hidden shadow-2xl border border-white/10 relative" v-show="showCamera">
        <video ref="videoRef" autoplay playsinline muted width="640" height="480" style="display: none;"></video>
    ...
    <div class="absolute bottom-8 left-1/2 transform -translate-x-1/2 flex items-center gap-4 z-20">
      <button @click="toggleMute" :class="['w-12 h-12 rounded-full flex items-center justify-center transition-colors', microphoneEnabled ? 'bg-white/20 hover:bg-white/30' : 'bg-red-500 hover:bg-red-600']">
      ...
      <button @click="toggleVideo" :class="['w-12 h-12 rounded-full flex items-center justify-center transition-colors', videoEnabled ? 'bg-white/20 hover:bg-white/30' : 'bg-red-500 hover:bg-red-600']">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <button @click="toggleScreenShare" :class="['w-12 h-12 rounded-full flex items-center justify-center transition-colors', isSharingScreen ? 'bg-green-500 hover:bg-green-600' : 'bg-white/20 hover:bg-white/30']" title="Udostępnij ekran">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          <path stroke-linecap="round" stroke-linejoin="round" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
        </svg>
      </button>
      ...
      <div class="relative">
        <button @click="filterMenuOpen = !filterMenuOpen" class="w-12 h-12 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition-colors" title="Efekty i filtry">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" />
          </svg>
        </button>

        <div v-if="filterMenuOpen" class="absolute bottom-16 left-1/2 transform -translate-x-1/2 bg-zinc-900 border border-zinc-800 rounded-xl p-2 flex flex-col gap-1 shadow-2xl w-44 z-30">
          <button
            @click="changeFilter('none')"
            :class="['text-xs text-left px-3 py-2 rounded-lg transition-colors', currentFilter === 'none' ? 'bg-white/10 text-white font-bold' : 'text-zinc-400 hover:bg-white/5 hover:text-white']"
          >
            Brak filtra
          </button>
          <button
            @click="changeFilter('blur')"
            :class="['text-xs text-left px-3 py-2 rounded-lg transition-colors', currentFilter === 'blur' ? 'bg-white/10 text-white font-bold' : 'text-zinc-400 hover:bg-white/5 hover:text-white']"
          >
            Rozmycie postaci
          </button>
          <button
            @click="changeFilter('image')"
            :class="['text-xs text-left px-3 py-2 rounded-lg transition-colors', currentFilter === 'image' ? 'bg-white/10 text-white font-bold' : 'text-zinc-400 hover:bg-white/5 hover:text-white']"
          >
            Wgraj własne tło...
          </button>
        </div>
      ...
      <button @click="handleDisconnect" class="w-12 h-12 bg-red-600 hover:bg-red-700 rounded-full flex items-center justify-center transition-colors shadow-lg">
