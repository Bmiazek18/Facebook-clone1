...
const notifications = ref([
  {
    id: 1,
    title: 'Nowe zapytanie o przedmiot',
    message: 'Użytkownik pyta, czy BMW series 7 E38 jest nadal dostępne.',
    time: '2 godziny temu',
    unread: true,
  },
  {
    id: 2,
    title: 'Twoje ogłoszenie zostało zaakceptowane',
    message: 'Ogłoszenie "Działka 30 arów" jest już widoczne dla innych.',
    time: '1 dzień temu',
    unread: false,
  },
])
      ...
      <div class="p-4">
        <div class="space-y-3">
          <div
            ...
            :key="notification.id"
            class="flex items-start p-3 rounded-lg hover:bg-theme-hover transition-colors cursor-pointer group relative"
          >
            <div class="flex-1">
              <div class="flex items-start justify-between">
                <div>
                  <h3 class="font-medium text-theme-text">{{ notification.title }}</h3>
                  <p class="text-sm text-theme-text-secondary mt-1">{{ notification.message }}</p>
                  <span class="text-xs text-theme-text-secondary mt-2 block">{{
                    notification.time
                  }}</span>
                </div>
                <div
                  v-if="notification.unread"
                  class="w-3 h-3 bg-theme-primary rounded-full ml-4"
                ></div>
              </div>
            </div>
